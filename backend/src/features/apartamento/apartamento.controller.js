...
exports.createApartamento = async (req, res) => {
  try {
    const { Tipo, NumeroApto, Piso, Tarifa, Estado } = req.body;
    if (!Tipo || NumeroApto === undefined || Piso === undefined || Tarifa === undefined || Estado === undefined) {
      return res.status(400).json({
        msg: "Todos los campos son requeridos: Tipo, NumeroApto, Piso, Tarifa, Estado"
      });
    }
    const apartamento = new Apartamento({ Tipo, NumeroApto, Piso, Tarifa, Estado });
...
exports.updateApartamento = async (req, res) => {
  try {
    ...
    const apartamento = await Apartamento.findByIdAndUpdate(
      req.params.id,
      { Tipo, NumeroApto, Piso, Tarifa, Estado },
      { new: true }
    );
